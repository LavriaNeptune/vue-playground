export default {
  ...
  /* props 在注册时可以更加细致地声明对传入 props 的校验需求...具体做法是为 props 选项提供一个带有 props 校验选项的对象, 参见:
    ...
    default -> 当未传入 prop 时的默认设定值,如果不对其进行设置,prop 默认的默认值均为 undefined(Boolean 类型的是例外,其默认值为 true) -> default 函数:如果默认值为引用类型数据,需要通过工厂函数提供默认值...该 default 函数会接受到组件所接受的原始 props 作为参数 PS
